...
var Cr = V(() => {
  "use strict";
  Ir();
  ys = typeof location > "u" ? void 0 : location.origin, Xn = import.meta.url > "file:" && import.meta.url < "file;", Tf = () => {
    if (true) {
      if (Xn) {
        let t = URL;
...
// src/offscreen.js
ye.wasm.numThreads = 1;
ye.wasm.wasmPaths = chrome.runtime.getURL("dist/");
var session = null;
async function getSession() {
  if (!session) {
    const modelUrl = chrome.runtime.getURL("model/model.onnx");
    session = await vf.create(modelUrl, {
...
async function solveCaptcha(dataUrl) {
  ...
  const inputTensor = new Ke("float32", tensor, [1, 3, 48, 256]);
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg.type === "offscreen-solve") {
    solveCaptcha(msg.dataUrl).then((answer) => sendResponse({ answer })).catch((err) => sendResponse({ error: err.message }));
    return true;
  }
});
